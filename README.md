...
The Professional Strategy Olympics execution-runtime placement is frozen by
[V010](docs/PROFESSIONAL_STRATEGY_OLYMPICS_EXECUTION_RUNTIME_V010.md). V010 is
design-only: it separates the exact authorized source from one external,
package-bound runtime and supersedes the unusable relative `.venv` command. It
does not implement an operator, runtime, authorization, or Olympics execution.
...
The merged [Authorization Governance V005](docs/PROFESSIONAL_STRATEGY_OLYMPICS_AUTHORIZATION_GOVERNANCE_V005.md) freezes the typed authorization lifecycle but remains a pure, non-executing validator. The additive, design-only [Operator Interface V006](docs/PROFESSIONAL_STRATEGY_OLYMPICS_OPERATOR_INTERFACE_V006.md) freezes the authorization-package, projection, ordering, and failure boundary behind the unchanged V005 command. The design-only [Runtime Boundary V007](docs/PROFESSIONAL_STRATEGY_OLYMPICS_RUNTIME_BOUNDARY_V007.md) prospectively resolves V006's remaining clock-bootstrap, socket, replay, repository-attestation, and future-implementation identity ambiguities without changing V005 or V006. The narrow, design-only [Clock Continuation V008](docs/PROFESSIONAL_STRATEGY_OLYMPICS_CLOCK_CONTINUATION_V008.md) freezes the sole nonce generator, sequence-2 continuation, durable evidence, lifecycle binding, interruption, replay, and rollback rules required before operator implementation. The design-only [Documentary Proof Transport V009](docs/PROFESSIONAL_STRATEGY_OLYMPICS_DOCUMENTARY_PROOF_TRANSPORT_V009.md) freezes one self-contained, package-bound, offline transport for the exact raw Git proof consumed by V005. No authorization, operator, verifier, attestor, or Olympics result exists.
...
Professional Strategy Benchmark Olympics V001 is frozen as an independent,
design-only comparison of ten mechanical intraday strategy families. Its
strategy contracts, shared simulator assumptions, two rankings, fifteen medal
events, advancement gates, multiple-testing controls, claim ladder, and blocked
readiness state are documented in
[`docs/PROFESSIONAL_STRATEGY_OLYMPICS_V001.md`](docs/PROFESSIONAL_STRATEGY_OLYMPICS_V001.md).
