"""Evidence and exploratory runner for VWAP Deviation Mean Reversion Child V001."""
...
SCHEMA = "aml.vwap-deviation-mean-reversion-child.v001"
MILESTONE_VERSION = "vwap-deviation-mean-reversion-prospective-child-v001"
EVIDENCE_MANIFEST_SCHEMA = "aml.vwap-deviation-mean-reversion-evidence-manifest.v001"
...
SYNTHETIC_RELATIVE_PATH = (
    "tests/fixtures/vwap_deviation_mean_reversion_child_v001/"
    "vwap_deviation_mean_reversion_synthetic.csv"
)
SOURCE_PATHS = (
    "scripts/run_vwap_deviation_mean_reversion_child_v001.py",
    "src/aml/benchmark_candidate_vwap_deviation_mean_reversion_v001.py",
    "src/aml/vwap_deviation_mean_reversion_child_v001.py",
)
...
class VwapDeviationMeanReversionMilestoneError(ValueError):
    """A candidate identity, evidence, or exploratory invariant failed."""


def required_inventory_contract() -> dict[str, object]:
    return {
        "schema_version": "aml.vwap-deviation-mean-reversion-required-inventory.v001",
...
def required_inventory_contract_identity() -> str:
    return canonical_hash(
        {
            "domain": "aml.vwap-deviation-mean-reversion-required-inventory.v001",
...
def default_config(repository_root: Path) -> dict[str, object]:
    ...
    value = {
        ...
        "synthetic_dataset_authorization": {
            "authorization_id": "vwap-deviation-mean-reversion-conformance-only-v001",
        ...
        "exploratory_dataset_binding": {
            ...
            "selection_rule": (
                ...
                "validator, with the first 20 reserved as selection warm-up and "
...
def validate_config(value: Mapping[str, object], repository_root: Path) -> dict[str, object]:
    ...
    if json.loads(canonical_json(value)) != expected:
        raise VwapDeviationMeanReversionMilestoneError("candidate configuration changed")
    if value.get("config_identity") != _config_identity(value):
        raise VwapDeviationMeanReversionMilestoneError("configuration identity changed")
    validate_dataset_authorization(
...
def _parent_entry(
    ...
    if library["library_identity"] != config["parent"]["library_identity"]:
        raise VwapDeviationMeanReversionMilestoneError("library identity changed")
    entry = next(
        (
            item
    ...
    if entry is None or any(
        ...
        raise VwapDeviationMeanReversionMilestoneError("immutable parent changed")
    ...
    if hypothesis["identity"] != PARENT_FRAMEWORK_HYPOTHESIS_IDENTITY:
        raise VwapDeviationMeanReversionMilestoneError("parent identity does not reproduce")
...
def _child_payload() -> dict[str, object]:
    return {
        ...
        "title": "Downside VWAP deviation with deceleration and reversal",
        "market_assumption": (
            "A large downside displacement from developing session VWAP can reflect "
            "temporary flow when the selloff visibly decelerates."
        ),
        "mechanism": (
            "Three strictly decelerating declines followed by a positive completed "
            "bar test convergence toward the signal-time frozen session VWAP."
        ),
        "required_evidence": [
            "consecutive regular-session minute OHLCV",
            "Wilder ATR20 warm-up",
            "developing regular-session HLC3-volume VWAP",
        ...
        "invalidation_conditions": [
            "VWAP downside deviation below 1.5 ATR20 at confirmation",
            "three immediately preceding declines are not strictly decelerating",
            "confirmation bar does not close above the prior close",
            "frozen VWAP target is not above the cost-adjusted next-bar entry",
            "required input or provenance unavailable",
        ],
        "known_risks": [
            "adverse selection",
            "VWAP follows rather than anchors a directional session",
            "deceleration is a pause before renewed downside continuation",
            "frozen VWAP remains unreachable after entry",
            "modeled costs exceed any gross effect",
        ],
        "required_indicators": [
            "Wilder ATR20",
            "regular-session HLC3-volume VWAP",
            "downside VWAP deviation normalized by ATR20",
            "three-bar close-decline deceleration",
        ],
        "expected_holding_period": "one to 60 complete one-minute bars",
        "expected_market_regime": "range-bound or two-sided intraday session",
        "expected_failure_modes": list(FROZEN_SPECIFICATION["expected_failure_modes"]),
        "taxonomy": ["mean-reversion", "price-volume", "vwap"],
        "contaminated_dataset_identities": [DATASET_FINGERPRINT],
        "multiple_testing_family": "liquidity-reversal-v001",
    }
...
def _lifecycle_conformance() -> dict[str, bool]:
    ...
    collision = replace(bars[25], high=200.0, low=90.0, close=99.0)
    collision_trades, collision_rejections = simulate_strategy(
        REFERENCE_STRATEGY_ID,
        [result.proposal],
        {("TEST", bars[0].session): (*bars[:25], collision)},
        calendar,
    )
    timeout_bars = [
        replace(
            bars[25],
            timestamp=bars[25].timestamp + timedelta(minutes=index),
            open=95.1,
            high=95.2,
            low=95.0,
            close=95.1,
    ...
    timeout_trades, timeout_rejections = simulate_strategy(
        REFERENCE_STRATEGY_ID,
        [result.proposal],
        {("TEST", bars[0].session): (*bars[:25], *timeout_bars)},
        calendar,
    )
    return {
        "lifecycle_timeout": (
...
def build_evidence(
    *, repository_root: Path, config: Mapping[str, object], library_path: Path
    ...
    observation = create_observation(
        {
            "observation_id": "vwap-deviation-mean-reversion-prospective-design-v001",
            "title": "Prospective human-authorized VWAP mean-reversion design",
    ...
    triage = create_triage(
        {
            ...
            "duplicate_signature": canonical_hash(
                {
                    "domain": "aml.vwap-deviation-mean-reversion-triage.v001",
            ...
            "reasons": [
                ...
                "The child adds a distinct VWAP-deviation mean-reversion mechanism.",
            ],
        },
        child,
    )
    ...
    reference = {
        ...
        "child_strategy_identity": CHILD_STRATEGY_IDENTITY,
        "reference_strategy_id": REFERENCE_STRATEGY_ID,
        "reference_contract": FROZEN_SPECIFICATION["reference_contract"],
        "executor_identity": EXECUTOR_IDENTITY,
        "specification_identity": SPECIFICATION_IDENTITY,
    ...
    binding = implementation_binding_artifact(
        ...
        implementation_callable=(
            "aml.benchmark_candidate_vwap_deviation_mean_reversion_v001."
            "evaluate_vwap_deviation_mean_reversion"
        ),
    ...
    conformance = run_conformance(
        ...
        cases=(
            ConformanceCase(
                "atr-warmup-unavailable",
                "unavailable",
                lambda: evaluate_vwap_deviation_mean_reversion(
                    inputs["atr-warmup-unavailable"]
                ),
            ),
            ConformanceCase(
                "confirmation-absent",
            ...
            ConformanceCase(
                ...
                "no_signal",
                lambda: evaluate_vwap_deviation_mean_reversion(
                    inputs["cooldown-active"]
                ),
            ),
            ConformanceCase(
                "deceleration-absent",
                "no_signal",
                lambda: evaluate_vwap_deviation_mean_reversion(inputs["negative"]),
            ),
            ConformanceCase(
                "duplicate-signal",
                "no_signal",
            ...
            ConformanceCase(
                ...
                "unavailable",
                lambda: evaluate_vwap_deviation_mean_reversion(inputs["unavailable"]),
            ),
            ConformanceCase(
                "positive",
                "proposal",
...
def verify_evidence_objects(
    ...
    if set(artifacts) != set(EVIDENCE_REQUIRED_ROLES):
        raise VwapDeviationMeanReversionMilestoneError("evidence inventory changed")
    validate_config(config, repository_root)
    ...
    if (
        ...
        raise VwapDeviationMeanReversionMilestoneError("conformance is incomplete")
    registration = artifacts["08-executor-registration.json"]
    if registration["payload"].get("registry_keys") != [CHILD_HYPOTHESIS_ID]:
        raise VwapDeviationMeanReversionMilestoneError("executor registration changed")
    if registration["payload"].get("implementation_binding_identity") != artifacts[
        ...
        raise VwapDeviationMeanReversionMilestoneError("executor lineage changed")
...
def write_evidence(
    ...
    if root.exists():
        raise VwapDeviationMeanReversionMilestoneError("evidence output already exists")
...
def verify_evidence_directory(
    ...
    *, repository_root: Path,
    ...
    if identity != canonical_hash(base) or not manifest.get("files"):
        raise VwapDeviationMeanReversionMilestoneError("evidence manifest changed")
    pairs = [
        (item.get("path"), item.get("role")) for item in manifest.get("files", [])
    ]
    if pairs != sorted(EVIDENCE_REQUIRED_ROLES.items()):
        raise VwapDeviationMeanReversionMilestoneError("evidence role inventory changed")
    artifacts: dict[str, Mapping[str, object]] = {}
    expected = {"manifest.json"}
    for item in manifest["files"]:
        relative = Path(str(item["path"]))
        if relative.is_absolute() or ".." in relative.parts:
            raise VwapDeviationMeanReversionMilestoneError("unsafe evidence path")
        path = root / relative
        if not path.is_file() or path.is_symlink() or _sha256(path) != item["sha256"]:
            raise VwapDeviationMeanReversionMilestoneError("evidence file hash changed")
        artifact = _strict_json(path)
        if artifact.get("identity") != item["identity"]:
            raise VwapDeviationMeanReversionMilestoneError("evidence identity changed")
    ...
    if actual != expected:
        raise VwapDeviationMeanReversionMilestoneError("evidence directory is not closed")
    verify_evidence_objects(artifacts, repository_root, config)
    return {
...
def _evaluate_exploratory(
    ...
    completed, rejections = simulate_strategy(
        REFERENCE_STRATEGY_ID, proposals, bars_by_key, calendar
    )
    if len(proposals) != len(completed) + len(rejections):
        raise VwapDeviationMeanReversionMilestoneError("proposal reconciliation failed")
    counts = {
...
def run_bounded_exploratory(
    *,
    ...
    if dataset.name != binding["dataset_vintage"]:
        raise VwapDeviationMeanReversionMilestoneError("dataset vintage changed")
    manifest_path = repository_root / str(binding["manifest_relative_path"])
    if _sha256(manifest_path) != binding["manifest_sha256"]:
        raise VwapDeviationMeanReversionMilestoneError("dataset manifest hash changed")
    source_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if source_manifest.get("dataset_fingerprint_sha256") != DATASET_FINGERPRINT:
        raise VwapDeviationMeanReversionMilestoneError("dataset fingerprint changed")
    ...
    run_identity = canonical_hash(
        {
            "domain": "aml.vwap-deviation-mean-reversion-exploratory-run.v001",
            ...
            "source_tree_identity": canonical_hash(
                {
                    "domain": "aml.vwap-deviation-mean-reversion-source-tree.v001",
    ...
    staging = Path(tempfile.mkdtemp(prefix=".vwap-deviation-v001-", dir=target.parent))
    try:
    ...
    verified = verify_exploratory_bundle(
        ...
        repository_root=repository_root,
        config=config,
        evidence_root=repository_root / "manifests/vwap_deviation_mean_reversion_child_v001",
    )
    return {
        "run_identity": run_identity,
...
def verify_exploratory_bundle(
    ...
    *,
    ...
    if identity != canonical_hash(base):
        raise VwapDeviationMeanReversionMilestoneError("exploratory manifest identity changed")
    if manifest.get("labels") != list(LABELS) or manifest.get(
        "candidate_specific_labels"
    ) != list(CANDIDATE_SPECIFIC_LABELS):
        raise VwapDeviationMeanReversionMilestoneError("exploratory labels changed")
    if manifest.get("required_inventory_contract_identity") != (
        required_inventory_contract_identity()
    ):
        raise VwapDeviationMeanReversionMilestoneError("inventory contract changed")
    pairs = [(item.get("path"), item.get("role")) for item in manifest.get("files", [])]
    if pairs != sorted(EXPLORATORY_REQUIRED_ROLES.items()):
        raise VwapDeviationMeanReversionMilestoneError("exploratory inventory changed")
    artifacts: dict[str, Mapping[str, object]] = {}
    expected = {"manifest.json"}
    for item in manifest["files"]:
        relative = Path(str(item["path"]))
        if relative.is_absolute() or ".." in relative.parts:
            raise VwapDeviationMeanReversionMilestoneError("unsafe exploratory path")
        ...
        if (
            ...
            raise VwapDeviationMeanReversionMilestoneError("exploratory artifact changed")
        artifact = _strict_json(artifact_path)
        if artifact.get("identity") != item.get("identity"):
            raise VwapDeviationMeanReversionMilestoneError("exploratory identity changed")
        if artifact.get("labels") != list(LABELS) or artifact.get(
            "candidate_specific_labels"
        ) != list(CANDIDATE_SPECIFIC_LABELS):
            raise VwapDeviationMeanReversionMilestoneError("artifact labels changed")
        _reject_candidate_prohibited_claims(
    ...
    if actual != expected:
        raise VwapDeviationMeanReversionMilestoneError("exploratory directory is not closed")
    result = artifacts["candidate_result"]
    validate_result(result)
    observations = result.get("qualitative_observations")
    if not isinstance(observations, list) or not observations:
        raise VwapDeviationMeanReversionMilestoneError("structured observations missing")
    ...
    if (
        ...
        raise VwapDeviationMeanReversionMilestoneError("observation reconciliation changed")
    ...
    if (
        ...
        raise VwapDeviationMeanReversionMilestoneError("exploratory lineage changed")
    counts = result["counts"]
    if counts["proposal_count"] != (
        counts["executed_trade_count"] + counts["rejected_proposal_count"]
    ):
        raise VwapDeviationMeanReversionMilestoneError("proposal counts do not reconcile")
