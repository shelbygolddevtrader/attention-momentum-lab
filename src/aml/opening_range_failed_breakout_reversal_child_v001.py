"""Evidence and exploratory runner for Opening Range Failed Breakout Reversal Child V001."""
...
from aml.benchmark_candidate_opening_range_failed_breakout_reversal_v001 import (
    ...
    conformance_bars,
    conformance_inputs,
    evaluate_opening_range_failed_breakout_reversal,
    evaluation_input,
...
SCHEMA = "aml.opening-range-failed-breakout-reversal-child.v001"
MILESTONE_VERSION = "opening-range-failed-breakout-reversal-prospective-child-v001"
EVIDENCE_MANIFEST_SCHEMA = "aml.opening-range-failed-breakout-reversal-evidence-manifest.v001"
...
SYNTHETIC_RELATIVE_PATH = (
    "tests/fixtures/opening_range_failed_breakout_reversal_child_v001/"
    "opening_range_failed_breakout_synthetic.csv"
)
SOURCE_PATHS = (
    ...
    "src/aml/opening_range_failed_breakout_reversal_child_v001.py",
)
...
class OpeningRangeFailedBreakoutMilestoneError(ValueError):
    """A candidate identity, evidence, or exploratory invariant failed."""


def required_inventory_contract() -> dict[str, object]:
    return {
        "schema_version": "aml.opening-range-failed-breakout-required-inventory.v001",
...
def required_inventory_contract_identity() -> str:
    return canonical_hash(
        {
            "domain": "aml.opening-range-failed-breakout-required-inventory.v001",
...
def default_config(repository_root: Path) -> dict[str, object]:
    ...
    value = {
        ...
        "synthetic_dataset_authorization": {
            "authorization_id": "opening-range-failed-breakout-conformance-only-v001",
...
def validate_config(value: Mapping[str, object], repository_root: Path) -> dict[str, object]:
    ...
    if json.loads(canonical_json(value)) != expected:
        raise OpeningRangeFailedBreakoutMilestoneError("candidate configuration changed")
    if value.get("config_identity") != _config_identity(value):
        raise OpeningRangeFailedBreakoutMilestoneError("configuration identity changed")
    validate_dataset_authorization(
...
def _parent_entry(
    ...
    if library["library_identity"] != config["parent"]["library_identity"]:
        raise OpeningRangeFailedBreakoutMilestoneError("library identity changed")
    entry = next(
        (
            item
    ...
    if entry is None or any(
        ...
        raise OpeningRangeFailedBreakoutMilestoneError("immutable parent changed")
    ...
    if hypothesis["identity"] != PARENT_FRAMEWORK_HYPOTHESIS_IDENTITY:
        raise OpeningRangeFailedBreakoutMilestoneError("parent identity does not reproduce")
...
def _child_payload() -> dict[str, object]:
    return {
        ...
        "title": "Five-minute opening-range failed downside breakout reclaim",
        "market_assumption": (
            "A downside opening-range excursion can be a temporary liquidity probe "
            "when the immediately adjacent bar closes back inside the range."
        ),
        "mechanism": (
            "The downside break can trap breakout participants; an adjacent reclaim "
            "with ordinary-or-greater same-clock participation tests a move toward "
            "the frozen opening-range midpoint."
        ),
        "required_evidence": [
        ...
        "invalidation_conditions": [
            "downside excursion below 0.25 times breakdown-bar ATR20",
            "breakdown close does not finish below the opening-range low",
            "adjacent bar does not close strictly back inside the opening range",
            "same-clock volume ratio below 1.0",
            "required input or provenance unavailable",
        ],
        "known_risks": [
            "adverse selection",
            "reclaim is a pause before renewed downside continuation",
            "opening-range midpoint remains unreachable after entry",
            "modeled costs exceed any gross effect",
        ],
        "required_indicators": [
            "Wilder ATR20",
            "five-minute opening-range high, low, and midpoint",
            "downside excursion normalized by ATR20",
            "same-clock volume median over twenty prior eligible sessions",
        ],
        "expected_holding_period": "one to 90 complete one-minute bars",
        "expected_market_regime": "liquidity-sweep or two-sided opening",
        "expected_failure_modes": list(FROZEN_SPECIFICATION["expected_failure_modes"]),
        "taxonomy": ["failed-breakout", "opening", "reversal"],
...
def build_evidence(
    *, repository_root: Path, config: Mapping[str, object], library_path: Path
    ...
    observation = create_observation(
        {
            "observation_id": "opening-range-failed-breakout-prospective-design-v001",
            "title": "Prospective human-authorized failed-breakout reversal design",
    ...
    triage = create_triage(
        {
            ...
            "duplicate_signature": canonical_hash(
                {
                    "domain": "aml.opening-range-failed-breakout-triage.v001",
            ...
            "reasons": [
                ...
                "The child adds a distinct opening-range reversal mechanism.",
            ],
        },
        child,
    )
    ...
    binding = implementation_binding_artifact(
        ...
        implementation_callable=(
            "aml.benchmark_candidate_opening_range_failed_breakout_reversal_v001."
            "evaluate_opening_range_failed_breakout_reversal"
        ),
    ...
    conformance = run_conformance(
        ...
        cases=(
            ConformanceCase(
                "adjacent-reclaim-absent",
                "no_signal",
                lambda: evaluate_opening_range_failed_breakout_reversal(
                    inputs["adjacent-reclaim-absent"]
                ),
            ),
            ConformanceCase(
                "duplicate-signal",
            ...
            ConformanceCase(
                "positive",
                "proposal",
                lambda: evaluate_opening_range_failed_breakout_reversal(inputs["positive"]),
            ),
            ConformanceCase(
                "volume-absent",
                "no_signal",
...
def verify_evidence_objects(
    ...
    if set(artifacts) != set(EVIDENCE_REQUIRED_ROLES):
        raise OpeningRangeFailedBreakoutMilestoneError("evidence inventory changed")
    validate_config(config, repository_root)
    ...
    if (
        ...
        raise OpeningRangeFailedBreakoutMilestoneError("conformance is incomplete")
    registration = artifacts["08-executor-registration.json"]
    if registration["payload"].get("registry_keys") != [CHILD_HYPOTHESIS_ID]:
        raise OpeningRangeFailedBreakoutMilestoneError("executor registration changed")
    if registration["payload"].get("implementation_binding_identity") != artifacts[
        ...
        raise OpeningRangeFailedBreakoutMilestoneError("executor lineage changed")
...
def write_evidence(
    ...
    if root.exists():
        raise OpeningRangeFailedBreakoutMilestoneError("evidence output already exists")
...
def verify_evidence_directory(
    ...
    *, repository_root: Path,
    ...
    if identity != canonical_hash(base) or not manifest.get("files"):
        raise OpeningRangeFailedBreakoutMilestoneError("evidence manifest changed")
    pairs = [
        (item.get("path"), item.get("role")) for item in manifest.get("files", [])
    ]
    if pairs != sorted(EVIDENCE_REQUIRED_ROLES.items()):
        raise OpeningRangeFailedBreakoutMilestoneError("evidence role inventory changed")
    artifacts: dict[str, Mapping[str, object]] = {}
    expected = {"manifest.json"}
    for item in manifest["files"]:
        relative = Path(str(item["path"]))
        if relative.is_absolute() or ".." in relative.parts:
            raise OpeningRangeFailedBreakoutMilestoneError("unsafe evidence path")
        path = root / relative
        if not path.is_file() or path.is_symlink() or _sha256(path) != item["sha256"]:
            raise OpeningRangeFailedBreakoutMilestoneError("evidence file hash changed")
        artifact = _strict_json(path)
        if artifact.get("identity") != item["identity"]:
            raise OpeningRangeFailedBreakoutMilestoneError("evidence identity changed")
    ...
    if actual != expected:
        raise OpeningRangeFailedBreakoutMilestoneError("evidence directory is not closed")
    verify_evidence_objects(artifacts, repository_root, config)
    return {
...
def _evaluate_exploratory(
    ...
    if len(proposals) != len(completed) + len(rejections):
        raise OpeningRangeFailedBreakoutMilestoneError("proposal reconciliation failed")
    counts = {
...
def run_bounded_exploratory(
    *,
    ...
    if dataset.name != binding["dataset_vintage"]:
        raise OpeningRangeFailedBreakoutMilestoneError("dataset vintage changed")
    manifest_path = repository_root / str(binding["manifest_relative_path"])
    if _sha256(manifest_path) != binding["manifest_sha256"]:
        raise OpeningRangeFailedBreakoutMilestoneError("dataset manifest hash changed")
    source_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if source_manifest.get("dataset_fingerprint_sha256") != DATASET_FINGERPRINT:
        raise OpeningRangeFailedBreakoutMilestoneError("dataset fingerprint changed")
    ...
    run_identity = canonical_hash(
        {
            "domain": "aml.opening-range-failed-breakout-exploratory-run.v001",
            ...
            "source_tree_identity": canonical_hash(
                {
                    "domain": "aml.opening-range-failed-breakout-source-tree.v001",
    ...
    staging = Path(tempfile.mkdtemp(prefix=".opening-range-failed-v001-", dir=target.parent))
    try:
    ...
    verified = verify_exploratory_bundle(
        ...
        repository_root=repository_root,
        config=config,
        evidence_root=repository_root / "manifests/opening_range_failed_breakout_reversal_child_v001",
    )
    return {
        "run_identity": run_identity,
...
def verify_exploratory_bundle(
    ...
    *,
    ...
    if identity != canonical_hash(base):
        raise OpeningRangeFailedBreakoutMilestoneError("exploratory manifest identity changed")
    if manifest.get("labels") != list(LABELS) or manifest.get(
        "candidate_specific_labels"
    ) != list(CANDIDATE_SPECIFIC_LABELS):
        raise OpeningRangeFailedBreakoutMilestoneError("exploratory labels changed")
    if manifest.get("required_inventory_contract_identity") != (
        required_inventory_contract_identity()
    ):
        raise OpeningRangeFailedBreakoutMilestoneError("inventory contract changed")
    pairs = [(item.get("path"), item.get("role")) for item in manifest.get("files", [])]
    if pairs != sorted(EXPLORATORY_REQUIRED_ROLES.items()):
        raise OpeningRangeFailedBreakoutMilestoneError("exploratory inventory changed")
    artifacts: dict[str, Mapping[str, object]] = {}
    expected = {"manifest.json"}
    for item in manifest["files"]:
        relative = Path(str(item["path"]))
        if relative.is_absolute() or ".." in relative.parts:
            raise OpeningRangeFailedBreakoutMilestoneError("unsafe exploratory path")
        ...
        if (
            ...
            raise OpeningRangeFailedBreakoutMilestoneError("exploratory artifact changed")
        artifact = _strict_json(artifact_path)
        if artifact.get("identity") != item.get("identity"):
            raise OpeningRangeFailedBreakoutMilestoneError("exploratory identity changed")
        if artifact.get("labels") != list(LABELS) or artifact.get(
            "candidate_specific_labels"
        ) != list(CANDIDATE_SPECIFIC_LABELS):
            raise OpeningRangeFailedBreakoutMilestoneError("artifact labels changed")
        _reject_candidate_prohibited_claims(
    ...
    if actual != expected:
        raise OpeningRangeFailedBreakoutMilestoneError("exploratory directory is not closed")
    result = artifacts["candidate_result"]
    validate_result(result)
    observations = result.get("qualitative_observations")
    if not isinstance(observations, list) or not observations:
        raise OpeningRangeFailedBreakoutMilestoneError("structured observations missing")
    ...
    if (
        ...
        raise OpeningRangeFailedBreakoutMilestoneError("observation reconciliation changed")
    ...
    if (
        ...
        raise OpeningRangeFailedBreakoutMilestoneError("exploratory lineage changed")
    counts = result["counts"]
    if counts["proposal_count"] != (
        counts["executed_trade_count"] + counts["rejected_proposal_count"]
    ):
        raise OpeningRangeFailedBreakoutMilestoneError("proposal counts do not reconcile")
