...
from aml.opening_range_failed_breakout_reversal_child_v001 import (
    ...
    OpeningRangeFailedBreakoutMilestoneError,
    build_evidence,
    default_config,
...
CONFIG = ROOT / "config/opening_range_failed_breakout_reversal_child_v001.json"
LIBRARY = ROOT / "config/benchmark_hypothesis_library_v001.json"
EVIDENCE = ROOT / "manifests/opening_range_failed_breakout_reversal_child_v001"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("excursion-absent", "no_signal", "downside_excursion_below_threshold"),
        ("adjacent-reclaim-absent", "no_signal", "adjacent_reclaim_absent"),
...
def test_frozen_decision_paths(case: str, status: str, reason: str | None) -> None:
    result = evaluate_opening_range_failed_breakout_reversal(conformance_inputs()[case])
...
def test_positive_proposal_uses_next_bar_entry_structure_stop_and_midpoint() -> None:
    result = evaluate_opening_range_failed_breakout_reversal(conformance_inputs()["positive"])
    assert result.proposal is not None
    proposal = result.proposal
    assert proposal.intended_entry_timestamp.endswith("09:52:00-05:00")
    assert proposal.stop == 98.44
    assert proposal.target > proposal.cost_adjusted_entry
    assert proposal.target == 100.0
    assert proposal.timeout_complete_bars == 90
...
def test_evidence_inventory_is_closed_and_write_once(tmp_path: Path) -> None:
    ...
    with pytest.raises(OpeningRangeFailedBreakoutMilestoneError, match="not closed"):
        verify_evidence_directory(target, repository_root=ROOT, config=_config())
    with pytest.raises(OpeningRangeFailedBreakoutMilestoneError, match="already exists"):
...
def test_evidence_rejects_missing_required_artifact(tmp_path: Path) -> None:
    ...
    with pytest.raises(OpeningRangeFailedBreakoutMilestoneError):
        verify_evidence_directory(target, repository_root=ROOT, config=_config())
