...
from aml.vwap_deviation_mean_reversion_child_v001 import (
    ...
    VwapDeviationMeanReversionMilestoneError,
    build_evidence,
    default_config,
...
CONFIG = ROOT / "config/vwap_deviation_mean_reversion_child_v001.json"
LIBRARY = ROOT / "config/benchmark_hypothesis_library_v001.json"
EVIDENCE = ROOT / "manifests/vwap_deviation_mean_reversion_child_v001"
...
def test_prospective_child_identities_and_registration_reproduce() -> None:
    ...
    assert FROZEN_SPECIFICATION["reference_contract"]["binding_kind"] == (
        "exact_semantic_alias"
    )
    assert STRATEGIES[REFERENCE_STRATEGY_ID]["strategy_identity"] == (
        REFERENCE_STRATEGY_IDENTITY
    )
    assert EXECUTOR_IDENTITIES[REFERENCE_STRATEGY_ID] == REFERENCE_EXECUTOR_IDENTITY
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("negative", "no_signal", "decline_sequence_not_strict"),
        ("extension-absent", "no_signal", "extension_below_threshold"),
        ("confirmation-absent", "no_signal", "positive_confirmation_absent"),
...
def test_frozen_decision_paths(case: str, status: str, reason: str | None) -> None:
    result = evaluate_vwap_deviation_mean_reversion(conformance_inputs()[case])
...
def test_positive_proposal_uses_next_bar_entry_structure_stop_and_frozen_vwap() -> None:
    result = evaluate_vwap_deviation_mean_reversion(conformance_inputs()["positive"])
    assert result.proposal is not None
    proposal = result.proposal
    assert proposal.intended_entry_timestamp.endswith("09:55:00-05:00")
    assert proposal.stop < 94.3
    assert proposal.target > proposal.cost_adjusted_entry
    frozen_vwap = float(dict(proposal.indicator_snapshots)["regular_vwap"])
    assert frozen_vwap <= proposal.target < frozen_vwap + 0.01
    assert proposal.timeout_complete_bars == 60


def test_missing_next_bar_fails_closed() -> None:
    bars = conformance_bars()
    result = evaluate_vwap_deviation_mean_reversion(
        evaluation_input(bars[:25], next_bar=None)
    )
    assert result.status == "unavailable"


def test_malformed_and_gapped_bars_raise_integrity_failure() -> None:
    bars = list(conformance_bars()[:25])
    bars[5] = replace(bars[5], high=bars[5].low - 1.0)
    malformed = evaluate_vwap_deviation_mean_reversion(
        evaluation_input(tuple(bars), next_bar=conformance_bars()[25])
    )
    assert malformed.status == "integrity_failure"
    bars = list(conformance_bars()[:25])
    bars[10] = replace(bars[10], timestamp=bars[10].timestamp + timedelta(minutes=1))
    gapped = evaluate_vwap_deviation_mean_reversion(
        evaluation_input(tuple(bars), next_bar=conformance_bars()[25])
    )
    assert gapped.status == "integrity_failure"
...
def test_evidence_inventory_is_closed_and_write_once(tmp_path: Path) -> None:
    ...
    with pytest.raises(VwapDeviationMeanReversionMilestoneError, match="not closed"):
        verify_evidence_directory(target, repository_root=ROOT, config=_config())
    with pytest.raises(VwapDeviationMeanReversionMilestoneError, match="already exists"):
...
def test_evidence_rejects_missing_required_artifact(tmp_path: Path) -> None:
    ...
    with pytest.raises(VwapDeviationMeanReversionMilestoneError):
        verify_evidence_directory(target, repository_root=ROOT, config=_config())
